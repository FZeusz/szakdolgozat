...
import { catColor, CAT_COLORS } from './shared';

// ── Kvíz kártya (csak Explore-hoz, nem mock) ──────────────────────
function ExploreCard({ quiz }) {
  ...
  return (
    <div className="quiz-card">
      ...
      <div className="quiz-card-meta">
        {quiz.time_limit ? <span>⏱ {Math.round(quiz.time_limit / 60)} perc</span> : <span>⏱ Nincs időkorlát</span>}
      </div>
...
export default function DashboardExplore() {
  ...
  // Kategória szűrő lehetőségek a betöltött kvízekből
  ...
  // Privát kvíz keresése kód alapján
  const handleFindPrivate = async (e) => {
    ...
    try {
      ...
      setFindError('Nem sikerült elérni a szervert!');
    } finally {
    ...
    <div className="tab-content">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-sub">Böngéssz mások által létrehozott nyilvános kvízek között, vagy keress privát kvízt megosztási kóddal.</p>
        </div>
      </div>
      ...
      <div className="private-find-box">
        <div className="private-find-header">
          ...
          <div>
            ...
            <div className="private-find-sub">Ha valaki megosztott veled egy kvízt, add meg a kódját itt.</div>
          </div>
        </div>
      ...
      {!loading && !error && filtered.length === 0 && (
        <div className="empty-state">
          ...
          <p>{search || catFilter !== 'Összes' ? 'Nincs találat a szűrőkre.' : 'Még nincs nyilvános kvíz más felhasználóktól.'}</p>
        </div>
      )}
      {!loading && !error && filtered.length > 0 && (
        <div className="quiz-grid">
          {filtered.map(q => <ExploreCard key={q.id} quiz={q} />)}
        </div>
      )}
